...
import today.yapeteam.font.Texture;
...
//    }
    ...
    public static void drawArc(MatrixStack matrices, float x, float y, float width, float height, float radius, float thickness, float start, float end, Color color1, Color color2) {
        preShaderDraw(matrices, x - width / 2f, y - height / 2f, x + width / 2f, y + height / 2f);
        ARC_PROGRAM.setParameters(x, y, width, height, radius, thickness, start, end, color1, color2);
        ARC_PROGRAM.use();
        Tessellator.getInstance().draw();
        endRender();
    }
